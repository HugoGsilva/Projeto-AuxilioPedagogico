...
| 2026-08-29 | Hugo | #67 | 3,0 | Convite de usuários: **só a diretora** convida (nome+email+papel) → link copiável (conta Pendente na tela de Usuários, regenerar/revogar) → página pública `/convite` valida token e a pessoa define a própria senha → já entra logada (sign-in no cliente reusando o login). Token forte só como hash no banco, uso único à prova de corrida, 7d, índice único parcial por email pending; aceite server-authoritative (mantém `disableSignUp`); erros genéricos (sem enumeração); auditoria `invitation.create/accept/revoke`. Substitui o cadastro-com-senha (`user.create` removido). Painel adversarial fechou: bug de FK que quebrava todo aceite, escalada do TI (novas permissões director-only `manageInvitations`/`assignRoles`, também na troca de papel), anti-DoS no hash e correções de UX. ADR-0008 |
...
| 2026-08-29 | Hugo | #71 | 0,5 | Bootstrap da conta "mãe" de diretora via `BOOTSTRAP_DIRECTOR_*` no start da API: idempotente por e-mail (nunca altera conta existente), mesmo caminho do aceite de convite (`user`+`account` com hash scrypt), auditoria `user.create` com `source: bootstrap_env`, recusa placeholders `TROCAR_*`; vars opcionais no schema de env e comentadas no stack do Portainer |
